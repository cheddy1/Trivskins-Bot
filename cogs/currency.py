...
class Currency(commands.Cog, name="Currency"):
    def __init__(self, bot):
        ...
        self.c.execute(
            """CREATE TABLE IF NOT EXISTS users (id integer PRIMARY KEY, name text NOT NULL, xp integer NOT NULL, xp_time integer NOT NULL)"""
        )
    ...
    async def on_message(self, ctx):
        ...
        if await valid_gain(
            self.c.execute(
                """SELECT xp_time FROM users WHERE id=?""", (user_id,)
            ).fetchone()[0]
        ):
            # Add xp to user (coins)
            self.c.execute(
                """UPDATE users SET xp = xp + ?, xp_time=? WHERE id=?""",
                (gain_amount, xp_time, user_id),
            )
            self.connection.commit()
    ...
    async def bal(self, ctx, *args):
        ...
        coins = self.c.execute(
            """SELECT xp FROM users WHERE id=?""", (user_id,)
        ).fetchone()
        if coins:
            # Create and send the bal embed message
            embed_desc = (
                user.mention + " currently has __**" + str(coins[0]) + "**__ coins!"
            )
            embed = discord.Embed(
                title="Coin Balance", description=embed_desc, color=0xA60B00
            )
    ...
    async def coins(self, ctx, *args):
        if "admin" in args:
            ...
            embed = discord.Embed(
                title="Admin Panel", description=embed_desc, color=0x2C3675
            )
        else:
            ...
            embed_shop = (
                "!buy ticket (#amount) (giveaway_name) - Buy giveaway entry tickets"
            )
            embed = discord.Embed(
                title="Coins Help", description=embed_desc, color=0x2C3675
            )
    ...
    async def leaderboard(self, ctx):
        ...
        embed = discord.Embed(
            title="Coins Leaderboard", description=embed_desc, color=0xFCBA03
        )
    ...
    async def highlow(self, ctx, *args):
        if not args:
            ...
            return await ctx.message.channel.send(
                "Missing amount of coins to gamble. `!coinflip (amount)`"
            )
        else:
            seed = random.randint(0, 100)
            embed_desc = (
                "M number number is **__"
                + str(seed)
                + "__**\nPress \U00002B06 if you think your number will be higher\nPress \U00002B07 if you think your number will be lower"
            )
            embed = discord.Embed(
                title="High Low", description=embed_desc, color=0xFCBA03
            )
            ...
            try:
                userchoice, user = await self.bot.wait_for(
                    "reaction_add",
    ...
    async def buy(self, ctx, item, amount, *args):
        if item == "ticket":
            try:
                ...
                await ctx.message.add_reaction("\U0000274C")
                return await ctx.message.channel.send(
                    "Invalid input. Try `!buy ticket (amount) (giveaway name)`"
                )
                ...
                if args:
                    ...
                    giveaway_name = str(
                        difflib.get_close_matches(
                            giveaway_name, result.split(","), 1, 0.4
                        )[0]
                    ).strip()
                else:
                    await ctx.message.add_reaction("\U0000274C")
                    return await ctx.message.channel.send(
                        "You must specify the giveaway name because there are multiple giveaways running currently. Use `!buy ticket (amount) (giveaway name)`\nCurrently open: **"
                        + result
                        + "**"
                    )
            ...
            if not giveaway_name:
                await ctx.message.add_reaction("\U0000274C")
                return await ctx.message.channel.send(
                    "A giveaway with that name does not exist! Currently running giveaways: "
                    + ", ".join(giveaways)
                )
            user_id = ctx.message.author.id
            giveaway_info = self.c.execute(
                """SELECT ticket_cost, ticket_max FROM giveaways WHERE name=?""",
                (giveaway_name,),
            ).fetchone()
            user_tickets = self.c.execute(
                """SELECT tickets FROM entries WHERE name=? AND id=?""",
                (giveaway_name, user_id),
            ).fetchone()
            total_cost = amount * int(giveaway_info[0])
            current_bal = self.c.execute(
                """SELECT xp FROM users WHERE id=?""", (user_id,)
            ).fetchone()
            while True:
                if not current_bal:
                    await ctx.message.add_reaction("\U0000274C")
                    return await ctx.message.channel.send(
                        "Please send a message before attempting to buy, you haven't been added to my database yet!"
                    )
                if total_cost > current_bal[0]:
                    ...
                    await ctx.message.add_reaction("\U0000274C")
                    return await ctx.message.channel.send(
                        "You do not have enough coins to buy any tickets, sorry!"
                    )
                else:
                    break
            if not user_tickets:
                ...
                if int(user_tickets[0]) >= giveaway_info[1]:
                    await ctx.message.add_reaction("\U0000274C")
                    return await ctx.message.channel.send(
                        "You already have the max number of tickets for this giveaway!"
                    )
                if (amount + user_tickets[0]) > giveaway_info[1]:
                    amount = int(giveaway_info[1])
                self.c.execute(
                    """UPDATE entries SET tickets = tickets + ? WHERE name=? AND id=?""",
                    (amount, giveaway_name, user_id),
                )
                self.connection.commit()
            self.c.execute(
                """UPDATE users SET xp = xp - ? WHERE id=?""", (total_cost, user_id)
            )
            self.connection.commit()
            user_tickets = self.c.execute(
                """SELECT tickets FROM entries WHERE name=? AND id=?""",
            ...
            entries = self.c.execute(
                """SELECT tickets FROM entries WHERE name=?""", (giveaway_name,)
            ).fetchall()
            num_tickets = 0
            ...
            await ctx.message.channel.send(
                ...
                + "**.\nYou have **"
                + str(user_tickets[0])
                + "** tickets for this giveaway. (Max of **"
                + str(giveaway_info[1])
                + "**)\nThere are **"
                + str(num_tickets)
                + "** entries so far in this giveaway."
            )
    ...
    async def editcoins(self, ctx, command, *args):
        ...
        if command.lower() == "give" and len(args) == 2:
            ...
            if self.c.execute(
                ...
                self.c.execute(
                    """UPDATE users SET xp = xp+? WHERE id=?""", (args[1], user_id)
                )
                self.connection.commit()
                return await ctx.message.add_reaction("\U00002705")
            else:
        ...
        # Sets a users balance to a certain amount
            ...
            if self.c.execute(
                ...
                self.c.execute(
                    """UPDATE users SET xp=? WHERE id=?""", (args[1], user_id)
                )
                self.connection.commit()
                return await ctx.message.add_reaction("\U00002705")
            else:
